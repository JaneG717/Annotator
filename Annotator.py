...
class Example(Frame):
    ...
    def initUI(self):
        ...
        self.RecommendModelName.grid(row=11, column=self.textColumn + 1, pady=4)
        self.RecommendModelFlag = Label(self, text=str(self.recommendFlag), foreground="red",
                                        font=(self.textFontStyle, 14, "bold"))
        self.RecommendModelFlag.grid(row=12, column=self.textColumn + 1, pady=4)
...
def getWordTagPairs(tagedSentence, seged=True, tagScheme="BIES", onlyNP=False, entityRe=r'\[\@.*?\#.*?\*\]'):
...
def turnFullListToOutputPair(fullList, seged=True, tagScheme="BIES", onlyNP=False):
    pairList = []
...
def outputWithTagScheme(input_list, label, tagScheme="BIES"):
    output_list = []
    text = input_list[0]
    list_length = len(text)
    if tagScheme == "BIES":
        if list_length == 1:
            pair = text[0] + ' ' + 'S-' + label + '\n'
            ...
            for idx in range(list_length):
                if idx == 0:
                    pair = text[idx] + ' ' + 'B-' + label + '\n'
                elif idx == list_length - 1:
                    pair = text[idx] + ' ' + 'E-' + label + '\n'
                else:
                    pair = text[idx] + ' ' + 'I-' + label + '\n'
                output_list.append(pair.encode('utf-8'))
    else:
        for idx in range(list_length):
            if idx == 0:
